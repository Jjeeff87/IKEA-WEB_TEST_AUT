...
- `TestersiteIkea.py` — pytest test class `TestIkeaSearch`. One shared `driver` for the whole class (`setup_class`/`teardown_class`); each test navigates fresh from the home page (`setup_method`) and flows through the page objects (home → search results → product) rather than calling Selenium directly.
